...
function MachineRing({
  ...
  const createSlots = (): ReactNode => {
    ...
    while (slotNumber < slotsPerReel) {
      let slot = machineItems
        ? getSlot(slotNumber, machineItems[slotNumber].icon)
        : console.log("items missing", machineItems);
      slots.push(slot);
      slotNumber++;
    }
    return slots;
  ...
  const getSlot = (i: number, icon: VMtypes.IconType): ReactNode => {
    ...
    let transFormStyle = {
      transform:
        ...
        slotAngle * i +
        "deg) translateZ(" +
        calcReelRadius(slotsPerReel) +
        "px)",
    };
    return (
      <div key={"slot " + i} style={transFormStyle} className={"slot " + i}>
        <Icon icon={icon} />
      </div>
    );
  };
  ...
  return <React.Fragment>{createRing(ringNumber)}</React.Fragment>;
}
